...
class AlphaScrollView extends StatefulWidget {
  const AlphaScrollView(
      {Key? key,
      required this.list,
  ...
  /// The itemBuilder must return a non-null widget and the third parameter id specifies
  /// the string mapped to this widget from the ```[list]``` passed.
  final Widget Function(BuildContext, int, String) itemBuilder;

  @override
  _AlphaScrollViewState createState() => _AlphaScrollViewState();
}

class _AlphaScrollViewState extends State<AlphaScrollView> {
  void init() {
    widget.list
    ...
    if (widget.isAlphabetsFiltered) {
      List<String> temp = [];
      for (var letter in _alphabetsData) {
        AlphaModel? firstAlphabetElement = _list.firstWhereOrNull(
            (item) => item.key.substring(2).toLowerCase().startsWith(letter.toLowerCase()));
      ...
      _filteredAlphabets = _alphabetsData;
    }
    calculateFirstIndex();
    setState(() {});
  }
  ...
  @override
  void didUpdateWidget(covariant AlphaScrollView oldWidget) {
  ...
  void calculateFirstIndex() {
    for (var letter in _filteredAlphabets) {
      AlphaModel? firstElement = _list.firstWhereOrNull(
          (item) => item.key.substring(2).toLowerCase().startsWith(letter));
...
//     _alphabetsData.add('$element');
//   }
//   return _alphabetsData;
// }
//
// List<String> alphabets = getData();
List<String> _alphabetsData = [
  '0',
  '1',
  '2',
  '3',
...
// List<int> listData = List.generate(10, (index) => index).toList();
